...
void main() {
  ...
  // Load normal from normal map in the range [0,1]
  vec4 tNormal = texture2D(uTexNormal, vTexCoord);
    
  // Convert normal from [0,1] to [-1,1]
  vec3 tNormalRGB = tNormal.rgb * 2.0 - 1.0;
    
  // Transform the normal to eye space and normalize
  vec3 transformedNormal = vNTMat[0].xyz * tNormalRGB.x + vNTMat[1].xyz * tNormalRGB.y + vNTMat[2].xyz * tNormalRGB.z;
