...
static const double CS380_PI = 3.14159265358979323846264338327950288;
static const double CS380_EPS = 1e-8;
static const double CS380_EPS2 = CS380_EPS * CS380_EPS;
static const double CS380_EPS3 = CS380_EPS * CS380_EPS * CS380_EPS;
...
public:
  ...
  Cvec& normalize() {
    assert(dot(*this, *this) > CS380_EPS2);
...
inline void softBind_cvec3(const Cvec3 &from, Cvec3 &to) {
  for (int i = 0; i < 3; i++)
  {
    to[i] = from[i];
  }
...
inline void softBind_cvec4(const Cvec4 &from, Cvec4 &to) { 
  for (int i = 0; i < 4; i++) {
    to[i] = from[i];
  }
  return;
