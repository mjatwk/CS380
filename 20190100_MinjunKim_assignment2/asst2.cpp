...
static Matrix4 g_objectRbt[2] = {Matrix4::makeTranslation(Cvec3(-1, 0, 0)), Matrix4::makeTranslation(Cvec3(1, 0, 0))}; // currently only 2 objs are defined
static Cvec3f g_objectColors[2] = {Cvec3f(1, 0, 0), Cvec3f(0, 1, 0)};

int g_vcount = 0;
int g_ocount = 1;
int g_mcount = 0;
static Matrix4 g_orjSkyRbt = Matrix4::makeTranslation(Cvec3(0.0, 0.25, 4.0));
static Matrix4 g_orjObjectRbt[2] = {Matrix4::makeTranslation(Cvec3(-1, 0, 0)), Matrix4::makeTranslation(Cvec3(1, 0, 0))}; // currently only 2 objs are defined

///////////////// END OF G L O B A L S //////////////////////////////////////////////////

static Matrix4 returnEyeRbt (int cnt) {
  switch (cnt % 3){
  case 0:
    return g_skyRbt;
    break;
...
static Matrix4& returnManipulateRbt(int cnt){
  switch (cnt % 3){
  case 0:
    return g_skyRbt;
    break;
...
static void drawStuff() {
  ...
  const Matrix4 eyeRbt = returnEyeRbt(g_vcount);
...
static void motion(const int x, const int y) {
  ...
  if (g_mouseClickDown) {
    Matrix4& l_objRbt = returnManipulateRbt(g_ocount);
    Matrix4& l_eyeRbt = returnManipulateRbt(g_vcount); 
    Matrix4 a = makeMixedFrame(l_objRbt, l_eyeRbt);
    Matrix4 temp;

    if (g_ocount % 3 == g_vcount % 3) {
      if (g_ocount % 3 == 0 && g_mcount % 2 == 1) {
        // perform ego motion
        Matrix4 l_orgRbt = Matrix4::makeTranslation(Cvec3(0.0, 0.0, 0.0));
        a = makeMixedFrame(l_orgRbt, l_eyeRbt); 
        temp = doMtoOwrtA(inv(m), l_eyeRbt, a);
        softBind(temp, l_objRbt);
      } else {
        // move the eye
        temp = doMtoOwrtA(inv(m), l_objRbt, a);
        softBind(temp, l_objRbt);
      }
    } else {
      // move the object
    ...
    // printMatrix(*returnManipulateRbt(g_ocount));
...
static void keyboard(const unsigned char key, const int x, const int y) {
  ...
  case 'r':
    ...
    memcpy(&g_objectRbt,&g_orjObjectRbt, sizeof(Matrix4)*2);
